...
class Robot:
    ...
    def step(self, num: int) -> None:
        ...
        if num == 0 and self.pos == [0, 0] and self.d == 0: self.d = 3
        while num:
            x, y = self.pos[0], self.pos[1]
            dx, dy = self.D[self.d]
            if self.d==0: most = self.w-x-1
            elif self.d==1: most = self.h-y-1
            elif self.d==2: most = x
            else: most = y
            
            step = min(num, most)
            self.pos = [x+dx*step, y+dy*step]
            if num>most: self.d = (self.d+1)%4
            num -= step
