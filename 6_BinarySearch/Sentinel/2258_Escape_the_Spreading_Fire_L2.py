    ...
    def maximumMinutes(self, A: List[List[int]]) -> int:
        ...
        def fire_bfs(Q):    
            while Q:
                ...
                for x, y, t in Q:
                    seen[(x, y)] = t
                    for dx, dy in D:
                        if 0<=x+dx<len(A) and 0<=y+dy<len(A[0]) and A[x+dx][y+dy] not in [1, 2] and t+1<seen[(x+dx, y+dy)]:
        ...
        def escape_bfs(t):
            ...
            while Q:
                nextQ = []
                for x, y, t in Q:
                    if x==len(A)-1 and y==len(A[0])-1: return t<=seen[(x, y)]
                    if seen[(x, y)]<=t: continue
                    
                    for dx, dy in D:
                        if 0<=x+dx<len(A) and 0<=y+dy<len(A[0]) and A[x+dx][y+dy] not in [1, 2] and (x+dx, y+dy) not in seen2:
                            seen2[(x+dx, y+dy)] = t+1
                            nextQ.append((x+dx, y+dy, t+1))
                Q = nextQ
            return False
