    ...
    def possibleToStamp(self, A: List[List[int]], H: int, W: int) -> bool:
        ...
        for i in range(m - H + 1):
            for j in range(n - W + 1):
                diff = prefix[i + H][j + W] - prefix[i + H][j] - \
                    prefix[i][j + W] + prefix[i][j]
                if diff == 0:
                    canPut[i][j] = 1
        ...
        for i in range(m):
            ...
            for j in range(n):
                jj = max(0, j - W + 1)
                if A[i][j] == 0 and prefix[i + 1][j + 1] - prefix[i + \
                    1][jj] - prefix[ii][j + 1] + prefix[ii][jj] == 0:
                    return False
        return True
