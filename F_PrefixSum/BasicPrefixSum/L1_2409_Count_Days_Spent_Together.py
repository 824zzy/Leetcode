...
class Solution:
    def countDaysTogether(self, arriveAlice: str, leaveAlice: str, arriveBob: str, leaveBob: str) -> int:
        ...
        start_m, start_d, end_m, end_d = int(start_m), int(start_d), int(end_m), int(end_d)
        
        return max(0, (prefix[end_m-1]+end_d)-(prefix[start_m-1]+start_d)+1)
...
class Solution:
    def countDaysTogether(self, arriveAlice: str, leaveAlice: str, arriveBob: str, leaveBob: str) -> int:
        ...
        start_m, start_d, end_m, end_d = int(start_m), int(start_d), int(end_m), int(end_d)
        if start_m==end_m: return max(end_d-start_d+1, 0)
        elif start_m>end_m: return 0
        else:
